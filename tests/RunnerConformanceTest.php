...
use Sifrious\Logres\WardrobeRunnerRuntime;
...
final class RunnerConformanceTest extends TestCase
{
    ...
    #[Test]
    public function runner_invokes_wardrobe_through_its_runtime_contract(): void
    {
        $adapter = new class implements RuntimeAdapter {
            public int $calls = 0;
            public function supports(string $runtime): bool { return $runtime === 'agent'; }
            public function invoke(RuntimeInvocation $invocation, RuntimeObserver $observer): RuntimeOutcome
            {
                ++$this->calls;
                $observer->event('status', ['message' => 'wardrobe']);
                $observer->stdout('ok');
                return new RuntimeOutcome('success', 0);
            }
        };
        [, , $events, $state] = $this->runner();
        $runtime = new WardrobeRunnerRuntime(['fake' => $adapter]);
        $runner = $this->runnerWith($runtime, $state, $events);
        $envelope = array_replace($this->envelope(), ['request_payload' => ['prompt' => 'return ok', 'timeout_seconds' => 30, 'permissions' => ['read']]]);

        $outcome = $runner->execute($envelope, $this->now());
