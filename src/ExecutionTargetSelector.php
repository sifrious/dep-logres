...
final class ExecutionTargetSelector
{
    ...
    public function select(ExecutionTargetRequirements $requirements, array $candidates, ExecutionTargetAuthorization $authorization, string $selectedAt, ?ExecutionTargetId $manualTargetId = null, ?string $overrideReason = null): TargetSelectionResult
    {
        ...
        foreach ($candidates as $candidate) {
            if (! $candidate instanceof ExecutionTargetCandidate) {
                throw new \InvalidArgumentException('Target inventory must contain only execution target candidates.');
            }
            $identity = $candidate->provider.':'.($candidate->providerTargetId ?? $candidate->id->value);
        ...
        $evaluations = array_map(function (ExecutionTargetCandidate $candidate) use ($requirements, $authorization, $selectedAt, $identityCounts): ExecutionTargetEvaluation {
            $identity = $candidate->provider.':'.($candidate->providerTargetId ?? $candidate->id->value);
            return $this->evaluate($requirements, $candidate, $authorization, $selectedAt, $identityCounts[$identity] > 1);
        }, $candidates);
    ...
    private function evaluate(ExecutionTargetRequirements $requirements, ExecutionTargetCandidate $candidate, ExecutionTargetAuthorization $authorization, string $selectedAt, bool $duplicateIdentity): ExecutionTargetEvaluation
    {
