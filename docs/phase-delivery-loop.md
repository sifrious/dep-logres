...
| T19 | Bind and normalize events | Logres + Funes history | Provider ack/envelope → binding, events, observations | Logres event stores + Funes SQL stores | Association checks + historical append auth | Ack/event/append identity dedupe | Raw envelope, normalized event, provenance | Logres-to-Funes adapter absent |
